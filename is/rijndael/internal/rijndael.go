package rijndael

import (
	"crypto/rand"
	"encoding/binary"
	"io"
)

// Количество раундов и размеры блока
const rounds = 14
const keyWords = 8
const BlockSize = 32
const blockWords = BlockSize / 4

// Cipher представляет собой структуру для шифра Rijndael
type Cipher struct {
	key  [blockWords * (rounds + 1)]uint32
	Iv   [32]byte
	Prev [32]byte
}

// NewCipher создает новый экземпляр Cipher на основе ключа
func NewCipher(key *[32]byte) *Cipher {
	c := new(Cipher)

	// Инициализация ключа
	for i := 0; i < keyWords; i++ {
		c.key[i] = binary.BigEndian.Uint32(key[4*i:])
	}

	// Раундовая константа и расширение ключа
	roundConstant := byte(1)
	for i := keyWords; i < blockWords*(rounds+1); i++ {
		temp := c.key[i-1]
		if i%keyWords == 0 {
			temp = subByte(rotByte(temp)) ^ (uint32(roundConstant) << 24)
			roundConstant = xtime(roundConstant)
		} else if i%keyWords == 4 {
			temp = subByte(temp)
		}
		c.key[i] = c.key[i-keyWords] ^ temp
	}

	// Инициализация вектора инициализации (IV)
	if _, err := io.ReadFull(rand.Reader, c.Iv[:]); err != nil {
		panic(err)
	}
	copy(c.Prev[:], c.Iv[:])

	return c
}

// Encrypt выполняет шифрование блока данных
func (c *Cipher) Encrypt(dst, src *[32]byte) {
	var state [4 * blockWords]byte
	copy(state[:], src[:])

	// Инициализация ключа
	addKey(&state, c.key[:])

	for round := 1; round < rounds; round++ {
		// ByteSub
		for i := range state {
			state[i] = sbox[state[i]]
		}

		// ShiftRow
		shiftRow(&state, 1, 1)
		shiftRow(&state, 2, 3)
		shiftRow(&state, 3, 4)

		// MixColumns
		for i := 0; i < blockWords; i++ {
			mixColumn(state[4*i:])
		}

		// Добавление раундового ключа
		addKey(&state, c.key[blockWords*round:])
	}

	// ByteSub
	for i := range state {
		state[i] = sbox[state[i]]
	}

	// ShiftRow
	shiftRow(&state, 1, 1)
	shiftRow(&state, 2, 3)
	shiftRow(&state, 3, 4)

	// Добавление последнего раундового ключа
	addKey(&state, c.key[blockWords*rounds:])

	copy(dst[:], state[:])
}

// Decrypt выполняет расшифрование блока данных
func (c *Cipher) Decrypt(dst, src *[32]byte) {
	var state [4 * blockWords]byte
	copy(state[:], src[:])

	// Инициализация последнего раундового ключа
	addKey(&state, c.key[blockWords*rounds:])

	// ShiftRow в обратном порядке
	shiftRow(&state, 1, -1)
	shiftRow(&state, 2, -3)
	shiftRow(&state, 3, -4)

	// ByteSub в обратном порядке
	for i := range state {
		state[i] = sboxInv[state[i]]
	}

	// Перебор раундов в обратном порядке
	for round := rounds - 1; round >= 1; round-- {
		// Добавление раундового ключа
		addKey(&state, c.key[blockWords*round:])

		// Инверсия MixColumns
		for i := 0; i < blockWords; i++ {
			mixColumnInv(state[4*i:])
		}

		// ShiftRow в обратном порядке
		shiftRow(&state, 1, -1)
		shiftRow(&state, 2, -3)
		shiftRow(&state, 3, -4)

		// ByteSub в обратном порядке
		for i := range state {
			state[i] = sboxInv[state[i]]
		}
	}

	// Инициализация первоначального ключа
	addKey(&state, c.key[:])

	copy(dst[:], state[:])
}

// subByte выполняет замену байтов в соответствии с S-box
func subByte(a uint32) uint32 {
	ret := uint32(0)
	for i := 0; i < 4; i++ {
		b := byte(a)
		ret >>= 8
		ret |= uint32(sbox[b]) << 24
		a >>= 8
	}
	return ret
}

// rotByte выполняет циклический сдвиг байтов
func rotByte(a uint32) uint32 {
	return (a >> 24) | (a << 8)
}

// addKey добавляет раундовый ключ к текущему состоянию
func addKey(state *[4 * blockWords]byte, key []uint32) {
	for i := 0; i < blockWords; i++ {
		t := binary.BigEndian.Uint32(state[4*i:])
		t ^= key[i]
		binary.BigEndian.PutUint32(state[4*i:], t)
	}
}

// shiftRow выполняет циклический сдвиг строк
func shiftRow(state *[4 * blockWords]byte, row, shift int) {
	var input [blockWords]byte
	for i := range input {
		input[i] = state[row+4*i]
	}

	src := shift
	if src < 0 {
		src += blockWords
	}
	dst := row
	for i := 0; i < blockWords; i++ {
		state[dst] = input[src]
		dst += 4
		dst %= blockWords * 4
		src++
		src %= blockWords
	}
}

// mixColumn выполняет операцию MixColumns
func mixColumn(col []byte) {
	a := col[0]
	b := col[1]
	c := col[2]
	d := col[3]
	e := a ^ b ^ c ^ d

	col[0] ^= e ^ xtime(a^b)
	col[1] ^= e ^ xtime(b^c)
	col[2] ^= e ^ xtime(c^d)
	col[3] ^= e ^ xtime(d^a)
}

// mixColumnInv выполняет обратную операцию MixColumns
func mixColumnInv(col []byte) {
	a := col[0]
	b := col[1]
	c := col[2]
	d := col[3]
	e := a ^ b ^ c ^ d
	z := xtime(e)
	x := e ^ xtime(xtime(z^a^c))
	y := e ^ xtime(xtime(z^b^d))

	col[0] ^= x ^ xtime(a^b)
	col[1] ^= y ^ xtime(b^c)
	col[2] ^= x ^ xtime(c^d)
	col[3] ^= y ^ xtime(d^a)
}

// xtime умножает байт на x в поле Галуа
func xtime(b byte) byte {
	c := uint32(b)
	c <<= 1
	c ^= ((c >> 8) & 1) * 0x1b
	return byte(c)
}

// FIPS-197 Figure 7. S-box substitution values in hexadecimal format.
var sbox = [256]byte{
	,
}

// FIPS-197 Figure 14.  Inverse S-box substitution values in hexadecimal format.
var sboxInv = [256]byte{
	,
}
